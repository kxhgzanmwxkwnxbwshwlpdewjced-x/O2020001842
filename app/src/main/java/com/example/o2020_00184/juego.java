...
//Imagen
    ...
    private ArrayList<Integer>  baraja (int longitud){
    ...
    private void commprobar (int i, final ImageButton imgb){
        if(primero == null){
            primero =imgb;
                    primero.setScaleType(ImageView.ScaleType.CENTER_CROP);
                    primero.setImageResource(imagenes[arrayDesordeando.get(i)]);
                    primero.setEnabled(false);
                    numeroPrimero = arrayDesordeando.get(i);
        }else{
            bloqueo = true;
            imgb.setScaleType(ImageView.ScaleType.CENTER_CROP);
            imgb.setScaleType(ImageView.ScaleType.CENTER_CROP);
            imgb.setEnabled(false);
            numeroSegundo = arrayDesordeando.get(i);
    ...
    private void init()
    {
        ...
        cargarTexto();
        cargarImagenes();
        arrayDesordeando = baraja(imagenes.length);
        for (int i=0; i< tablero.length; i++){
          tablero[i].setScaleType(ImageView.ScaleType.CENTER_CROP);
          tablero[i].setImageResource(imagenes[arrayDesordeando.get(i)]);
        }
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
